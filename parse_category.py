...
from tululu_download import download_txt, download_image, parse_book_page
...
def download_many_books(soup, json_path, dest_folder, genre_id, skip_txt, skip_imgs):
    ...
    for book_href in soup.select('div.bookimage a'):
        try: 
            ...
            book_info = {'title': parsed_page['book_title'],
            ...
            book_info_json = json.dumps(book_info, indent=4, ensure_ascii=False)                 
            with open(json_path_and_name, 'a', encoding='utf_8') as my_file:
                my_file.write(f'{book_info_json}\n') 
            if not skip_imgs: 
